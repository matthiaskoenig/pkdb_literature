...
def get_pdfs_for_bib(source: Path, target_dir: Path):
    """
    source: bib file
    ...
    pdf_dif = target_dir / 'pdf'
    pdf_dif.mkdir(parents=True, exist_ok=True)

    for reference in bib_database.entries:
        if 'file' not in reference:
            if 'doi' in reference:
                url = reference['doi']
                file_name = pdf_dif / f"{reference['ID']}.pdf"
                ...
                if 'err' not in result:
                    with open(file_name, 'wb+') as fd:
                        fd.write(result["pdf"])
                        reference['file'] = f"pdf/{reference['ID']}.pdf"

                    target = target_dir / f"{source.name[:-4]}_updated.bib"
                    db.entries.append(reference)
                    with open(target, 'w') as bibfile:
                        bibfile.write(writer.write(db))
                else:
                    rich.print(reference['ID'])
                    rich.print(result)
